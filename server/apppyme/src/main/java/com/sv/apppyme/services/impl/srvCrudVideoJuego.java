...
public class srvCrudVideoJuego implements ICRUDVideoJuego {
	...
	public boolean updateImgDelJuegoSucces(VideoJuegoDto videojuegoInfo, String key) {
		...
		if (ImgDao.update(img).getCodigo() != Constantes.SUCCES)
			return false;
		log.info("[" + key + "]" + "::::[updateImgDelJuegoSucces]::: Imagen actualizado en base de datos correctamente:::");
		return true;
	}
	public boolean updateJuegoSucces(VideoJuegoDto videojuegoInfo, String key) {
		...
		log.info("[" + key + "]" + "::::[updateJuegoSucces]::: videojuego actualizado en base de datos correctamente:::");
		return true;
	}
	...
	public boolean deleteImgDelJuegoSucces(VideoJuegoDto videojuegoInfo, String key) {
		...
		if (ImgDao.delete(img).getCodigo() != Constantes.SUCCES)
			return false;
		log.info("[" + key + "]" + "::::[deleteImgDelJuegoSucces]::: Imagen eliminada en base de datos correctamente:::");
		return true;
	}
	...
	public GenericEntityResponse<Videojuego> getOneByIDVideojuego(VideoJuegoDto videojuegoInfo)
		...
		if(resDao.getCodigo() != Constantes.SUCCES) {
			throw new SrvValidacionException(Constantes.ERROR, "Error en el proceso de obtener el videojuego con id " + videojuegoInfo.getId());
		}
		
		v = setearAtributosVideoJuegosIndividuales(resDao.getEntity());
	...
	public Videojuego setearAtributosVideoJuegosIndividuales(Videojuego v) throws SrvValidacionException {
		
		GenericEntityResponse<Categoria> resCat = categoriaDao.getOneById(v.getCategoria().getId());
		if(resCat.getCodigo() != Constantes.SUCCES)
			throw new SrvValidacionException(Constantes.ERROR, "Error en el proceso de obtener la categoria");
		...
		if(respla.getCodigo() != Constantes.SUCCES)
			throw new SrvValidacionException(Constantes.ERROR, "Error en el proceso de obtener la plataforma");

		GenericEntityResponse<Img> resImg = imgDao.getOneById(v.getImg().getId());
		if(resImg.getCodigo() != Constantes.SUCCES)
			throw new SrvValidacionException(Constantes.ERROR, "Error en el proceso de obtener la img");
		...
		if(resDesa.getCodigo() != Constantes.SUCCES)
			throw new SrvValidacionException(Constantes.ERROR, "Error en el proceso de obtener la desarrolladora");
		...
		log.info(":::::[setearAtributosfetOneById]::::Atributos setteados correctamente::::");
		return v;
	}

	@Override
	public GenericEntityResponse<List<Videojuego>> getAllVideojuego()
			throws SrvValidacionException {
		...
		for(Videojuego v: resDao.getEntity()) {
			ls.add(setearAtributosVideoJuegosIndividuales(v));
		}
