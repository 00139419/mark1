package com.sv.apppyme.utils;

public class Mensajeria {
	
	public static String MJS_ERROR_CREANDO_JWT = "Ocurrio un error interno!";
	public static String MJS_ERROR_PARSEAR_OBJECT_TO_STRING = "Ocurrio un error con la data!";
	public static String MJS_ERROR_ENCRIPTAR_TEXTO = "Ocurrio un error de seguridad!";
	public static String MJS_LOGIN_ERROR = "";
	public static String MJS_ERROR_GENERICO (String errorMessage) {return "Ocurrio un error genrico: " + errorMessage;};
	public static String MJS_CONTRASEÑA_INCORRECTA = "Usuario o contraseña invalidos!";

}
